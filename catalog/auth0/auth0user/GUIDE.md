...
- **The connection must be enabled for at least one application, or Auth0 refuses the user.** A database connection with an empty `enabledClients` is "disabled" in Auth0's eyes, and creating a user in it fails at create with `400 Bad Request: The connection is disabled` on both engines -- a true sentence about the connection, reported on the user. Enable the connection for a client (an `Auth0Client` by reference on the `Auth0Connection`) before declaring users in it; a connection declared only for its own sake has none.
