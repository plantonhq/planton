...
| `bootstrap.sh` | Renders the placeholders (plus the per-bootstrap `PLANTON_E2E_GKE_BATCH_ID`, reused on re-runs), adopts the permanent KMS key ring back into its node's state when a previous batch left it in the project (its teardown is an abandon, so the next bootstrap's create would 409), applies the set, reads the Mongo key and the R2 token's S3 pair from the set lane's node state, writes the kubeconfig and `env.sh` (mode 600) |
