...
| `vault.auto_unseal` | no | built-in seal | Seal the vault with a key in your cloud — `aws_kms`, `gcp_kms` (by reference to the catalog's KMS kinds and the seal identity), `azure_key_vault`, or `transit` — so it unseals itself on every start, including after a restore; the key and its grants exist before the platform (the seal is checked at start; GCP needs both KMS roles). Credentials never land in the resource: the module materializes them as a Secret (`<platform>-openbao-seal-creds`) the operator hands to the vault as environment variables |
