terraform {
  required_providers {
    ...
    # Google publishes Firebase's core resources -- the app registration and
    # its configuration lookup -- ONLY in the beta provider; the GA provider
    # has no code for them at the pin. That resource (and its data source)
    # alone attach `provider = google-beta` below, under a recorded admission
...
# a quota project". The override attributes quota to the project the app is
# registered in under every credential mode. Both provider blocks carry it
# so behavior never depends on which channel a resource rides.
