...
|-------|---------|------------|
| `chatEngineConfig` | create an agent | `dialogflowAgentToLink` -- a `GcpDialogflowCxAgent` reference or a literal agent name (with `allowCrossRegion` if the agent lives elsewhere) -- to answer through an agent you manage instead; preset 05 shows the reference. |
