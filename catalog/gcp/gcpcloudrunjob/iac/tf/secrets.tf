...
#     template's service_account, else the project's Compute Engine default).
# The variable reads the exact version, so a new value updates the template.

locals {
  # Keyed "<container index>/<variable>" -- the address the env block in
  # main.tf looks each variable up by.
  env_secrets = merge([
    for container_index, container in var.spec.template.containers : {
...
# identity Cloud Run runs tasks as when the template names none.
...
resource "google_secret_manager_secret_version" "env" {
  ...
  # A new value creates its version before the old one goes, so a running
  # execution keeps a readable version until the template moves.
  lifecycle {
