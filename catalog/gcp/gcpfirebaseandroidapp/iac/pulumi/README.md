# GCP Firebase Android App - Pulumi Module

## Overview

This directory contains the Pulumi implementation for registering an Android app in a Firebase-enabled Google Cloud project using Planton's `GcpFirebaseAndroidApp` API. The module is written in Go and creates `firebase.AndroidApp` — the registration — plus the composed `firebase.AppCheckPlayIntegrityConfig` (when configured) and `firebase.AppCheckDebugToken` (per spec entry), with `projects.Service` for the App Check API, then reads the app's `google-services.json` through `firebase.GetAndroidAppConfigOutput`.
...
- Registers the app with its display name, immutable package name, certificate fingerprints (sent only when declared), the API key UID (sent only when set — Firebase associates or provisions one otherwise), and the spec's `deletion_policy`.
- Enables `firebaseappcheck.googleapis.com` exactly when the spec composes an App Check resource (never disabled on destroy), then creates the Play Integrity configuration (when its block is present and not declared off) and one debug token per entry, keyed by display name, each waiting on the registration and the API. Token values are marked secret in state.
...
- **The Play Integrity configuration is never deleted on Google's side**; removing it from the spec forgets it, and destroying the app removes it with the app.
