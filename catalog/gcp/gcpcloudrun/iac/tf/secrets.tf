...
#     spec's service_account, else the project's Compute Engine default).
# The variable reads the exact version, so a new value stamps a new revision.

locals {
  env_secret_regions = (
    var.spec.region == "global" && var.spec.multi_region_settings != null
  ) ? var.spec.multi_region_settings.regions : [var.spec.region]

  # Keyed "<container index>/<variable>" -- the address the env block below
  # looks each variable up by.
  env_secrets = merge([
...
# identity Cloud Run runs as when the spec names none.
...
resource "google_secret_manager_secret" "env" {
  ...
  replication {
    user_managed {
      dynamic "replicas" {
        for_each = local.env_secret_regions
        content {
