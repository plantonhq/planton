...
Nothing running signs in to the vault with its root token. At initialization the operator enables the vault's Kubernetes auth method (configured with the in-cluster API server's name and nothing else, so it reads the vault pod's own certificate and token and keeps working on whatever cluster a restored vault runs in), writes two policies and two roles, and from then on signs in on every pass as its own ServiceAccount -- a session created for the pass and revoked when it ends, never stored anywhere. The vault verifies that sign-in with the cluster through its own ServiceAccount, which the operator grants the cluster's `system:auth-delegator` role by a ClusterRoleBinding labeled with the platform's UID and collected by the operator's janitor when the platform is gone (a cluster-scoped object cannot be owned by a namespaced one). The control plane gets a token of its own, minted through a token role and limited to the `secret/` and `transit/` engines -- nothing under `sys/` or `auth/` -- into `<platform>-openbao-token`, the one credential Secret the operator rewrites rather than creates once: it looks the token up by its accessor every pass, renews it when less than half of its seven-day period is left, mints a new one when the Secret is missing or the token is no longer valid (a platform restored into a fresh namespace, a token somebody revoked, an operator away longer than the period), and rolls the control plane on the new accessor. The operator's own policy is the platform's objects in the vault -- the two engines' mounts, its policies and roles, minting and renewing through the role -- and never the engines themselves. The root token stays in the init Secret as the break-glass: the operator uses it for exactly one thing, repairing its own sign-in when the vault does not know the operator's current identity (an operator reinstalled under another name or namespace), and when the vault refuses the operator and no root token is at hand, the status names the identity the operator has, the binding to check, and the two ways back. An open vault whose init Secret has gone missing is a working platform with a hazard named in its Ready sentence -- the break-glass gone under a cloud seal, the next restart unopenable under the built-in one -- never a refusal. The vault's PostgreSQL connection reaches the server as the driver's own environment (`PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`, `PGSSLMODE`, and `PGPASSWORD` projected from the role's Secret) with no connection URL anywhere, the same way the catalog's standalone vault hands its connection over.
...
A component signs in to what it manages as itself, and hands each consumer a credential of the consumer's own. The vault is the case: the operator's access is a session it opens with its own Kubernetes identity and closes at the end of the pass, so there is nothing to store, rotate, or restore; the consumer's access is a scoped token the operator issues, keeps alive, and issues again after a restore, projected from a Secret the way every consumer already reads its credentials. A master credential that exists (the vault's root token) is the break-glass, used only to repair the operator's own sign-in, and no running component depends on it.
