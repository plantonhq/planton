...
message KubernetesTemporalDynamicConfig {
  ...
  optional int64 history_size_limit_error = 1 [(dev.planton.shared.options.default) = "52428800"];
  
  // Maximum number of events in workflow execution history
  // Default: 51200. Increase for workflows with many activities/signals.
  optional int64 history_count_limit_error = 2 [(dev.planton.shared.options.default) = "51200"];
...
message KubernetesTemporalSpec {
  ...
  optional int32 num_history_shards = 13 [(dev.planton.shared.options.default) = "512"];
}
```
...
message KubernetesTemporalServiceConfig {
  // Number of replicas
  optional int32 replicas = 1 [(dev.planton.shared.options.default) = "1"];
...
- OpenMCF [80/20 design philosophy](https://github.com/plantonhq/planton/blob/main/architecture/README.md#the-8020-principle)
...
Fixed in catalog `v0.1.0` (`4711a8ce7e`). `KubernetesTemporalSpec` carries all three things asked for: `dynamic_config` (history size and count limits with their warning thresholds, plus blob size limits), `num_history_shards` (default 512, validated), and `services`, which sets replicas and resources for the frontend, history, matching and worker services separately. The Pulumi module maps them into the chart's `server.dynamicConfig` and per-service values.

One difference from the ask: `max_concurrent_workflow_task_pollers` is not a typed field. It is a worker-side setting, and any other dynamic-config key rides `helm_values` under `server.dynamicConfig`.

This file sat in an untracked folder the repository ignores (`bazel-planton/`), so it is recorded here for the first time as it closes.
