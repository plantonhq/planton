...
Currently, after deploying an Auth0Client via Planton, operators must **manually log into the Auth0 Dashboard** to:
...
In `apis/dev/planton/provider/auth0/auth0client/v1/iac/pulumi/module/client.go`, add:
...
In `apis/dev/planton/provider/auth0/auth0client/v1/iac/tf/main.tf`, add:
...
spec:
  ...
  grantTypes:
    - client_credentials
  apiGrants:
    - audience: "https://planton-preprod.us.auth0.com/api/v2/"
      scopes:
        - read:users
        - read:user_idp_tokens
```
...
spec:
  ...
  grantTypes:
    - client_credentials
  apiGrants:
    - audience: "https://planton.us.auth0.com/api/v2/"
      scopes:
        - read:users
        - update:users
        - delete:users
...
Users must:
- Deploy Auth0Client via Planton
