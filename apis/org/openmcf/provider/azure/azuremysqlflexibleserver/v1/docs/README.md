...
OpenMCF's AzureMysqlFlexibleServer component provides:
...
consistent high I/O performance, though this is not exposed in the OpenMCF v1 spec (80/20).
...
- `iops` and `io_scaling_enabled` are available but not exposed in OpenMCF v1 (80/20)
...
consistent with all other Azure modules in OpenMCF.
...
Every Azure resource in OpenMCF requires `resource_group` (StringValueOrRef) and `region` (string).
...
is important for teams managing both, and for ensuring the OpenMCF components
