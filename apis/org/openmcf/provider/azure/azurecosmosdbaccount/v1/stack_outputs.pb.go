// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: org/openmcf/provider/azure/azurecosmosdbaccount/v1/stack_outputs.proto

package azurecosmosdbaccountv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// **AzureCosmosdbAccountStackOutputs** captures the outputs of provisioning
// an Azure Cosmos DB account with its databases and containers/collections.
//
// The primary outputs are `account_id` for resource identification, `endpoint`
// for SDK connectivity, and `primary_key`/`primary_connection_string` for
// authentication. The `database_ids` map provides individual database resource
// IDs for downstream references.
//
// Unlike relational databases (PostgreSQL, MySQL, MSSQL) which use login/password
// authentication, Cosmos DB uses auto-generated access keys. Applications connect
// using the `endpoint` + `primary_key` or the `primary_connection_string`.
//
// In the database-stack infra chart, the `account_id` output is referenced by
// AzurePrivateEndpoint for establishing private connectivity. The
// `primary_connection_string` or `primary_mongodb_connection_string` is consumed
// by applications for database access.
type AzureCosmosdbAccountStackOutputs struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The Azure Resource Manager ID of the Cosmos DB account.
	// Format: /subscriptions/{sub}/resourceGroups/{rg}/providers/Microsoft.DocumentDB/databaseAccounts/{name}
	//
	// Referenced by AzurePrivateEndpoint (private_connection_resource_id)
	// for establishing private connectivity.
	// Subresource names for private endpoint: ["Sql"] for SQL API, ["MongoDB"] for Mongo API.
	AccountId string `protobuf:"bytes,1,opt,name=account_id,json=accountId,proto3" json:"account_id,omitempty"`
	// The name of the Cosmos DB account.
	AccountName string `protobuf:"bytes,2,opt,name=account_name,json=accountName,proto3" json:"account_name,omitempty"`
	// The document endpoint URI.
	// Format: https://{name}.documents.azure.com:443/
	//
	// Used by Cosmos DB SDKs to connect to the account.
	// Combined with `primary_key` for authentication.
	Endpoint string `protobuf:"bytes,3,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	// The primary access key for the account.
	// Used for key-based authentication with the Cosmos DB SDK.
	//
	// For production workloads, consider using RBAC-based authentication
	// (Azure AD) instead of access keys.
	PrimaryKey string `protobuf:"bytes,4,opt,name=primary_key,json=primaryKey,proto3" json:"primary_key,omitempty"`
	// The primary connection string for the SQL/NoSQL API.
	// Format: AccountEndpoint={endpoint};AccountKey={key};
	//
	// Ready-to-use connection string for SQL API SDKs.
	// Always populated regardless of account kind.
	PrimaryConnectionString string `protobuf:"bytes,5,opt,name=primary_connection_string,json=primaryConnectionString,proto3" json:"primary_connection_string,omitempty"`
	// The primary connection string for the MongoDB API.
	// Format: mongodb://{name}:{key}@{name}.mongo.cosmos.azure.com:10255/?ssl=true&...
	//
	// Only populated when `kind` is "MongoDB".
	// Empty string for GlobalDocumentDB accounts.
	PrimaryMongodbConnectionString string `protobuf:"bytes,6,opt,name=primary_mongodb_connection_string,json=primaryMongodbConnectionString,proto3" json:"primary_mongodb_connection_string,omitempty"`
	// Map of database names to their Azure Resource Manager IDs.
	// Includes both SQL databases and MongoDB databases, depending on the
	// account kind.
	// Format: { "mydb": "/subscriptions/.../sqlDatabases/mydb" }
	DatabaseIds   map[string]string `protobuf:"bytes,7,rep,name=database_ids,json=databaseIds,proto3" json:"database_ids,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AzureCosmosdbAccountStackOutputs) Reset() {
	*x = AzureCosmosdbAccountStackOutputs{}
	mi := &file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AzureCosmosdbAccountStackOutputs) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AzureCosmosdbAccountStackOutputs) ProtoMessage() {}

func (x *AzureCosmosdbAccountStackOutputs) ProtoReflect() protoreflect.Message {
	mi := &file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AzureCosmosdbAccountStackOutputs.ProtoReflect.Descriptor instead.
func (*AzureCosmosdbAccountStackOutputs) Descriptor() ([]byte, []int) {
	return file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescGZIP(), []int{0}
}

func (x *AzureCosmosdbAccountStackOutputs) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetAccountName() string {
	if x != nil {
		return x.AccountName
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetPrimaryKey() string {
	if x != nil {
		return x.PrimaryKey
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetPrimaryConnectionString() string {
	if x != nil {
		return x.PrimaryConnectionString
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetPrimaryMongodbConnectionString() string {
	if x != nil {
		return x.PrimaryMongodbConnectionString
	}
	return ""
}

func (x *AzureCosmosdbAccountStackOutputs) GetDatabaseIds() map[string]string {
	if x != nil {
		return x.DatabaseIds
	}
	return nil
}

var File_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto protoreflect.FileDescriptor

const file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDesc = "" +
	"\n" +
	"Forg/openmcf/provider/azure/azurecosmosdbaccount/v1/stack_outputs.proto\x122org.openmcf.provider.azure.azurecosmosdbaccount.v1\"\xf3\x03\n" +
	" AzureCosmosdbAccountStackOutputs\x12\x1d\n" +
	"\n" +
	"account_id\x18\x01 \x01(\tR\taccountId\x12!\n" +
	"\faccount_name\x18\x02 \x01(\tR\vaccountName\x12\x1a\n" +
	"\bendpoint\x18\x03 \x01(\tR\bendpoint\x12\x1f\n" +
	"\vprimary_key\x18\x04 \x01(\tR\n" +
	"primaryKey\x12:\n" +
	"\x19primary_connection_string\x18\x05 \x01(\tR\x17primaryConnectionString\x12I\n" +
	"!primary_mongodb_connection_string\x18\x06 \x01(\tR\x1eprimaryMongodbConnectionString\x12\x88\x01\n" +
	"\fdatabase_ids\x18\a \x03(\v2e.org.openmcf.provider.azure.azurecosmosdbaccount.v1.AzureCosmosdbAccountStackOutputs.DatabaseIdsEntryR\vdatabaseIds\x1a>\n" +
	"\x10DatabaseIdsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\xa7\x03\n" +
	"6com.org.openmcf.provider.azure.azurecosmosdbaccount.v1B\x11StackOutputsProtoP\x01Zkgithub.com/plantonhq/openmcf/apis/org/openmcf/provider/azure/azurecosmosdbaccount/v1;azurecosmosdbaccountv1\xa2\x02\x05OOPAA\xaa\x022Org.Openmcf.Provider.Azure.Azurecosmosdbaccount.V1\xca\x022Org\\Openmcf\\Provider\\Azure\\Azurecosmosdbaccount\\V1\xe2\x02>Org\\Openmcf\\Provider\\Azure\\Azurecosmosdbaccount\\V1\\GPBMetadata\xea\x027Org::Openmcf::Provider::Azure::Azurecosmosdbaccount::V1b\x06proto3"

var (
	file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescOnce sync.Once
	file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescData []byte
)

func file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescGZIP() []byte {
	file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescOnce.Do(func() {
		file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDesc), len(file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDesc)))
	})
	return file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDescData
}

var file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_goTypes = []any{
	(*AzureCosmosdbAccountStackOutputs)(nil), // 0: org.openmcf.provider.azure.azurecosmosdbaccount.v1.AzureCosmosdbAccountStackOutputs
	nil,                                      // 1: org.openmcf.provider.azure.azurecosmosdbaccount.v1.AzureCosmosdbAccountStackOutputs.DatabaseIdsEntry
}
var file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_depIdxs = []int32{
	1, // 0: org.openmcf.provider.azure.azurecosmosdbaccount.v1.AzureCosmosdbAccountStackOutputs.database_ids:type_name -> org.openmcf.provider.azure.azurecosmosdbaccount.v1.AzureCosmosdbAccountStackOutputs.DatabaseIdsEntry
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_init() }
func file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_init() {
	if File_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDesc), len(file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_goTypes,
		DependencyIndexes: file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_depIdxs,
		MessageInfos:      file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_msgTypes,
	}.Build()
	File_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto = out.File
	file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_goTypes = nil
	file_org_openmcf_provider_azure_azurecosmosdbaccount_v1_stack_outputs_proto_depIdxs = nil
}
