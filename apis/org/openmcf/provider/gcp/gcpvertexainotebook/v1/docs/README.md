...
OpenMCF targets the current v2 API via `google_workbench_instance` (Terraform) and `workbench.Instance` (Pulumi).
...
OpenMCF provides a declarative YAML interface with cross-resource composition, framework labels, and dual IaC backend support.
