...
When you deploy a GcpFirestoreDatabase resource, OpenMCF provisions:
...
**CMEK via `kms_key_name` (not `cmek_config` sub-message):** We flatten the CMEK configuration to a single `kms_key_name` field for consistency with other OpenMCF components (GcpSpannerDatabase, GcpBigQueryDataset, GcpBigtableInstance). The Pulumi/Terraform modules wrap it in the appropriate `cmek_config`/`CmekConfig` structure internally.
