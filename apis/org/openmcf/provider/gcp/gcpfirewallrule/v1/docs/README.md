...
Pulumi brings general-purpose programming languages to infrastructure management. A firewall rule in Pulumi (Go, matching the OpenMCF implementation language):
...
OpenMCF takes a cleaner approach by separating the **action** from the **rules**:
...
The `project_id` and `network` fields use OpenMCF's `StringValueOrRef` pattern, which enables two modes:
...
2. **Foreign key reference**: Reference another OpenMCF resource's output, creating a dependency chain.
...
OpenMCF's protobuf schema encodes GCP's constraints directly:
...
- **Tag/SA mutual exclusion**: GCP does not allow mixing tag-based targeting with service-account-based targeting in the same rule. OpenMCF validates this at the schema level, catching the error before any API call is made.
...
OpenMCF's `GcpFirewallRuleSpec` covers the fields that appear in the vast majority of real-world firewall rules:
...
| `enable_logging` (boolean) | GCP's API supports both a boolean `enable_logging` and a structured `log_config`. OpenMCF uses only `log_config` (presence = enabled), which is the modern, recommended approach. |
...
| `deny` + `allow` in the same rule | GCP doesn't support this. OpenMCF's action+rules design makes this impossible by construction. |
...
1. **Stop clicking**: Move every firewall rule to code, whether that's Terraform, Pulumi, or OpenMCF manifests.
2. **Validate early**: Catch misconfigurations at authoring time, not deployment time. OpenMCF's schema-level validations (direction, action, source requirements, tag/SA mutual exclusion) prevent entire categories of errors.
