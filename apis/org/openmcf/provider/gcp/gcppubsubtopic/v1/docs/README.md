...
|---------|-------|-------------|
| Created by | Infrastructure (OpenMCF) | Infrastructure (OpenMCF) |
...
OpenMCF models topics and subscriptions as separate resources because they have
...
| OpenMCF (this component) | Yes | Separate resource | Yes | Yes | Yes | Yes |

OpenMCF adds cross-resource composability that Terraform and Pulumi lack
natively -- topics can reference projects, KMS keys, and GCS buckets from
other OpenMCF resources using `valueFrom`.

### Terraform Mapping

| OpenMCF Field | Terraform Attribute |
|---------------|-------------------|
| `projectId` | `project` |
| `topicName` | `name` |
...
Pub/Sub topics support GCP labels. The OpenMCF framework applies standard labels:

- `openmcf-resource: true`
- `openmcf-resource-name: <topic_name>`
- `openmcf-resource-kind: gcppubsubtopic`
