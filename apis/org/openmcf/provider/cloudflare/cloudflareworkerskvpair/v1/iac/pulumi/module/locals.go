package module

import (
	cloudflareprovider "github.com/plantonhq/openmcf/apis/org/openmcf/provider/cloudflare"
	cloudflareworkerskvpairv1 "github.com/plantonhq/openmcf/apis/org/openmcf/provider/cloudflare/cloudflareworkerskvpair/v1"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// Locals bundles handy references for the rest of the module.
type Locals struct {
	CloudflareProviderConfig *cloudflareprovider.CloudflareProviderConfig
	CloudflareWorkersKvPair  *cloudflareworkerskvpairv1.CloudflareWorkersKvPair
}

func initializeLocals(_ *pulumi.Context, stackInput *cloudflareworkerskvpairv1.CloudflareWorkersKvPairStackInput) *Locals {
	locals := &Locals{}
	locals.CloudflareWorkersKvPair = stackInput.Target
	locals.CloudflareProviderConfig = stackInput.ProviderConfig
	return locals
}
