package module

import (
	cloudflareprovider "github.com/plantonhq/openmcf/apis/org/openmcf/provider/cloudflare"
	cloudflareemailroutingaddressv1 "github.com/plantonhq/openmcf/apis/org/openmcf/provider/cloudflare/cloudflareemailroutingaddress/v1"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// Locals bundles handy references for the rest of the module.
type Locals struct {
	CloudflareProviderConfig      *cloudflareprovider.CloudflareProviderConfig
	CloudflareEmailRoutingAddress *cloudflareemailroutingaddressv1.CloudflareEmailRoutingAddress
}

func initializeLocals(_ *pulumi.Context, stackInput *cloudflareemailroutingaddressv1.CloudflareEmailRoutingAddressStackInput) *Locals {
	locals := &Locals{}
	locals.CloudflareEmailRoutingAddress = stackInput.Target
	locals.CloudflareProviderConfig = stackInput.ProviderConfig
	return locals
}
