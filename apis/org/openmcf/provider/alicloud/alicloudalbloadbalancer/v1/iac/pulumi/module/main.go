...
func Resources(ctx *pulumi.Context, stackInput *alicloudalbloadbalancerv1.AlicloudAlbLoadBalancerStackInput) error {
	locals := initializeLocals(ctx, stackInput)
	spec := locals.AlicloudAlbLoadBalancer.Spec
	...
	if lbName == "" {
		lbName = locals.AlicloudAlbLoadBalancer.Metadata.Name
	}
