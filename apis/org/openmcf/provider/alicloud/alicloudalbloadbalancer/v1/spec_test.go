...
func TestAlicloudAlbLoadBalancerSpec(t *testing.T) {
	gomega.RegisterFailHandler(ginkgo.Fail)
	ginkgo.RunSpecs(t, "AlicloudAlbLoadBalancerSpec Validation Tests")
}
...
func minimalValidSpec() *AlicloudAlbLoadBalancerSpec {
	return &AlicloudAlbLoadBalancerSpec{
		Region: "cn-hangzhou",
		VpcId:  strRef("vpc-abc123"),
		ZoneMappings: []*AlicloudAlbZoneMapping{
...
func minimalValidInput() *AlicloudAlbLoadBalancer {
	return &AlicloudAlbLoadBalancer{
		ApiVersion: "alicloud.openmcf.org/v1",
		Kind:       "AlicloudAlbLoadBalancer",
		Metadata:   &shared.CloudResourceMetadata{Name: "test-alb"},
		Spec:       minimalValidSpec(),
	}
}

var _ = ginkgo.Describe("AlicloudAlbLoadBalancerSpec Validation Tests", func() {
	...
	ginkgo.Describe("valid input", func() {
		...
		ginkgo.It("should pass with all top-level optional fields populated", func() {
			...
			input.Spec.AccessLogConfig = &AlicloudAlbAccessLogConfig{
		...
		ginkgo.It("should pass with server groups and listeners", func() {
			input := minimalValidInput()
			input.Spec.ServerGroups = []*AlicloudAlbServerGroup{
				{
					Name: "web-backend",
					HealthCheckConfig: &AlicloudAlbHealthCheckConfig{
						HealthCheckEnabled: true,
					},
				},
			}
			input.Spec.Listeners = []*AlicloudAlbListener{
				{
					ListenerPort:                 80,
			...
			input.Spec.ServerGroups = []*AlicloudAlbServerGroup{
				{
					...
					StickySessionConfig: &AlicloudAlbStickySessionConfig{
	...
	ginkgo.Describe("invalid input", func() {
		...
		ginkgo.It("should fail when zone_mappings has fewer than 2 entries", func() {
			input := minimalValidInput()
			input.Spec.ZoneMappings = []*AlicloudAlbZoneMapping{
				{ZoneId: "cn-hangzhou-a", VswitchId: strRef("vsw-aaa")},
			}
			err := protovalidate.Validate(input)
			gomega.Expect(err).ToNot(gomega.BeNil())
