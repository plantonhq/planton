...
This document examines the full deployment landscape and explains how OpenMCF bundles the security group with its rules into a single validated API resource.
...
OpenMCF v1 creates normal security groups, which cover the 80% use case. Enterprise support can be added in v2 for high-rule-count scenarios.
...
Unlike AWS (which separates ingress and egress into structural lists), Alibaba Cloud treats direction as a field on the rule. OpenMCF follows the provider's native model: a single `rules` list with a `type` field for direction. This matches both the Terraform/Pulumi resource model and the underlying ECS API.
