...
func Resources(ctx *pulumi.Context, stackInput *alicloudnlbloadbalancerv1.AlicloudNlbLoadBalancerStackInput) error {
	locals := initializeLocals(ctx, stackInput)
	spec := locals.AlicloudNlbLoadBalancer.Spec
	...
	if lbName == "" {
		lbName = locals.AlicloudNlbLoadBalancer.Metadata.Name
	}
