...
| OpenMCF | Backend state | Yes | Yes (via backend) | Manifest references | Yes | Manifest VCS |
...
**Bandwidth as `int32`, not string**: The Alibaba Cloud provider accepts bandwidth as a string (`"10"`), but the OpenMCF proto defines it as `optional int32` with `gte: 1, lte: 1000` validation. This provides:
...
**Defaults via proto annotations**: The three optional fields with defaults use `(org.openmcf.shared.options.default)` annotations:
...
This is intentionally minimal. The component's value is not in complexity but in providing a standardized, validated, tagged EIP allocation that integrates cleanly with the rest of the OpenMCF resource graph.
...
**Pre-allocate before dependent resources**: When building a stack (VPC → EIP → NAT → ACK), allocate the EIP before the NAT gateway. This ensures the EIP exists when the NAT gateway's `eip_id` reference resolves. OpenMCF's dependency graph handles this automatically, but manual operators should be aware of the ordering.
