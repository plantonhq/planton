...
- **`repeated StringValueOrRef` for `security_group_ids`** -- First component in OpenMCF to use repeated FK fields. Each element independently resolves as a literal UUID or `value_from` reference, enabling InfraChart DAG wiring to multiple security groups.
