# OpenStackKeypair Pulumi Module — Architecture Overview

## Data Flow

```
manifest.yaml
  └─> OpenStackKeypairStackInput (proto)
...
1. **main.go (entrypoint)**: Loads `OpenStackKeypairStackInput` from the `STACK_INPUT` environment variable (base64-encoded YAML), then calls `module.Resources`.
