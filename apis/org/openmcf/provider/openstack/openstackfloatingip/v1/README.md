...
Allocate and optionally associate Neutron floating IPs in OpenStack using OpenMCF's unified API.
...
Floating IPs in OpenStack do not have a name attribute. The `metadata.name` is used for OpenMCF identity only.
...
Reference an external network managed by OpenMCF:
