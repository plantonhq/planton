...
The Auth0 provider warns against mixing `auth0_role_permissions` (manages the complete list) with `auth0_role_permission` (appends one). Choosing the authoritative resource gives declarative, drift-correcting behavior consistent with the rest of OpenMCF: the manifest is the source of truth, and a permission removed from the spec is removed from the role on the next apply. The append-style single-permission resource is intentionally not modeled.
