...
The **AwsEventBridgeRule** resource provides a standardized way to provision and manage AWS EventBridge rules with bundled targets through OpenMCF. It supports event pattern matching, schedule-based triggering, input transformation, per-target dead letter queues, and retry policies.
...
When you define an AwsEventBridgeRule resource, OpenMCF:
...
5. **Applies Tags**: Tags the rule with OpenMCF metadata (organization, environment, resource kind, resource ID).
