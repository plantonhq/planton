...
This document provides in-depth technical context for the `AwsAppRunnerService` OpenMCF component. It is intended for contributors, platform engineers, and anyone who needs to understand the design decisions behind the component spec.
...
| --- | --- | --- | --- |
| API | `"API"` | Inline in the OpenMCF spec (`runtime`, `build_command`, `start_command`, `port`) | Simple apps where you want all config in one place |
...
AWS Copilot is a CLI that deploys to ECS Fargate (or App Runner). It is an **orchestration tool**, not a compute service. You can use Copilot to deploy App Runner services, but OpenMCF serves a similar orchestration role with a declarative manifest approach.
