...
Comprehensive technical documentation for the AwsMskCluster deployment component in OpenMCF. This document covers architecture, networking, authentication, encryption, storage, monitoring, logging, configuration, cost, limits, security, common patterns, and the v2 roadmap.
...
MSK Configuration holds Apache Kafka `server.properties` overrides. In OpenMCF, there are two ways to configure:
...
2. **External (`configurationArn` + `configurationRevision`):** References a pre-existing MSK Configuration created outside OpenMCF.
