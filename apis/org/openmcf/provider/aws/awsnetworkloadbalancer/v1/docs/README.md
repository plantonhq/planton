...
This document provides an architecture-focused research and design reference for the AwsNetworkLoadBalancer OpenMCF component. It covers Layer 4 load balancing fundamentals, NLB-specific behavior, operational constraints, and production patterns.
...
**NLB constraint**: For NLB target groups, `unhealthy_threshold` must equal `healthy_threshold` (unlike ALB). The OpenMCF IaC modules enforce this.
...
When `dns.enabled` is true, OpenMCF creates Route53 **alias** A records pointing your hostnames to the NLB's DNS name. Alias records:
