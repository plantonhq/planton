...
The **AwsSqsQueue** resource provides a standardized way to provision and manage AWS SQS queues through OpenMCF. It supports both Standard and FIFO queue types with dead letter queue routing, server-side encryption (SSE-SQS or customer-managed KMS), IAM access policies, and fine-grained delivery tuning.
...
When you define an AwsSqsQueue resource, OpenMCF:
...
5. **Applies Tags**: Tags the queue with OpenMCF metadata (organization, environment, resource kind, resource ID).
