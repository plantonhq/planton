...
`KubernetesTelemetry` OpenMCF component (kind 866). Telemetry is the mesh's observability
...
Upstream spec fields are flattened after the OpenMCF namespaced envelope (`target_cluster`,
...
carried as a plain `string` (no OpenMCF validation), faithful to upstream.
