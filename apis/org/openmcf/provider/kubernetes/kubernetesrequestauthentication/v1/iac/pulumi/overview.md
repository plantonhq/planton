...
- **Spec is assigned by value** -- `RequestAuthenticationArgs.Spec` is a `PtrInput` satisfied by the `RequestAuthenticationSpecArgs` value itself; the `*Ptr()` wrapper marshals to the wrong element type and panics at apply (a bug the PeerAuthentication forge caught live). Nested `Selector` / `TargetRefs` / `JwtRules` follow the same value-assignment rule.
