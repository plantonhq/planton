...
**Version distinction (do not conflate):** the OpenMCF kind's own API version is `v1`
(`cloud_resource_kind.proto` `version: v1`, package directory `kubernetesenvoyfilter/v1/`),
identical for every OpenMCF kind. The **Istio CRD** is served at `networking.istio.io/v1alpha3`
