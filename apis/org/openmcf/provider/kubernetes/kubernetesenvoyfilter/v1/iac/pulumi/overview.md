...
- **Spec is assigned by value** -- `EnvoyFilterArgs.Spec` is a `PtrInput` satisfied by the `EnvoyFilterSpecArgs` value itself; the `*Ptr()` wrapper marshals to the wrong element type and panics at apply.
