...
- **Spec is assigned by value** -- `ServiceEntryArgs.Spec` is a `PtrInput` satisfied by the `ServiceEntrySpecArgs` value itself; the `*Ptr()` wrapper marshals to the wrong element type and panics at apply.
