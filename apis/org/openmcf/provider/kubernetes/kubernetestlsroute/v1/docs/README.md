...
Gateway API `TLSRoute` into the OpenMCF component model, and the design decisions
...
6. Validation Rules
7. Why TLSRoute Is a First-Class OpenMCF Component
8. Design Decisions
...
`KubernetesTlsRoute` brings that resource into OpenMCF as a first-class deployment
...
The OpenMCF spec flattens the upstream `TLSRouteSpec` after the standard
...
Upstream expresses the SNI no-IP rule as `self.all(h, !isIP(h))`. OpenMCF's
...
## Why TLSRoute Is a First-Class OpenMCF Component
...
- **DD-008 value modeling.** Open sets use the upstream regex; no proto enums and
  no baked-in OpenMCF defaults on upstream fields.
...
`KubernetesTlsRoute` completes the TLS-passthrough slice of the OpenMCF Gateway
