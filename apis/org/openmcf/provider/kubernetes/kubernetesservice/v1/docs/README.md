...
This document provides a comprehensive analysis of the Kubernetes Service landscape, explains why OpenMCF offers a standalone Service component, and justifies the design decisions behind the `KubernetesServiceSpec` schema.
...
Workload components in OpenMCF (`KubernetesDeployment`, `KubernetesStatefulSet`) automatically create Services for their pods. However, a standalone `KubernetesService` component addresses scenarios that fall outside the workload lifecycle:
...
4. **Cross-tool integration**: Services for pods managed by Helm charts, operators, or other tools outside OpenMCF
...
OpenMCF provides a unified manifest format that works with both Pulumi and Terraform, plus:
...
**Enum conventions**: Service types use lowercase values (`cluster_ip`, `node_port`, `load_balancer`, `external_name`) following OpenMCF enum guidelines. Protocol values (`TCP`, `UDP`, `SCTP`) use uppercase as they represent external standards where uppercase is conventional.
