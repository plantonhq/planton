...
upstream Gateway API `ReferenceGrant` into the OpenMCF component model, and the
...
5. The from / to Model
6. Validation Rules
7. Why ReferenceGrant Is a First-Class OpenMCF Component
8. Design Decisions
...
`KubernetesReferenceGrant` brings that resource into OpenMCF as a first-class
...
cross-namespace references. It is the seventh and final member of the OpenMCF
...
The OpenMCF spec flattens the upstream `ReferenceGrantSpec` after the standard
...
## Why ReferenceGrant Is a First-Class OpenMCF Component
...
- **`group` / `kind` are plain `string`, `to.name` is `optional string` (DD-008,
  ...
  `optional`.
- **No baked-in OpenMCF defaults on upstream fields (dont-do #4).** Upstream
...
found the design hard to settle and left it for the future). OpenMCF therefore
...
2. **Topology dependencies use `metadata.relationships`.** The `from`/`to` entries
   ...
   when it is OpenMCF-managed, the author expresses that edge explicitly:

```yaml
metadata:
  name: "{{ values.env }}-allow-frontend"
  relationships:
    - kind: KubernetesNamespace          # the source ("from") namespace, if OpenMCF-managed
...
`KubernetesReferenceGrant` completes the OpenMCF Gateway API ingress layer at 100%
