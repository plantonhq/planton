...
```bash
openmcf pulumi up --manifest cluster-issuer.yaml --stack org/project/env
```
...
3. The ClusterIssuer is named after the `dns_domain` value (e.g., `example.com`), matching the convention all OpenMCF ingress components use to derive issuer names from hostnames

## Naming Convention

The ClusterIssuer Kubernetes resource is named after the `dns_domain` field. This is critical because all OpenMCF ingress-enabled components (KubernetesDeployment, KubernetesArgocd, KubernetesKafka, etc.) derive the issuer name from the ingress hostname by stripping the first label:
