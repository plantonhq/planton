...
Gateway API `GRPCRoute` into the OpenMCF component model, and the design
...
6. Validation Rules
7. Why GRPCRoute Is a First-Class OpenMCF Component
8. Design Decisions
...
`KubernetesGrpcRoute` brings that resource into OpenMCF as a first-class
...
The OpenMCF spec flattens the upstream `GRPCRouteSpec` after the standard
...
## Why GRPCRoute Is a First-Class OpenMCF Component
...
- **DD-008 value modeling.** Closed enums use CEL `in`; open sets use the upstream
  regex; no proto enums and no baked-in OpenMCF defaults on upstream fields.
...
OpenMCF provisions the standard-channel GRPCRoute. Excluded experimental fields:
...
`KubernetesGrpcRoute` completes the gRPC half of the OpenMCF Gateway API ingress
