...
The `name` field exists because OCI policy names have different constraints than OpenMCF metadata names — OCI allows spaces and special characters that DNS-compatible metadata names do not. Users who need policy names matching existing OCI naming conventions can set `name` explicitly.
...
Consistent with all OCI components in OpenMCF, policies receive freeform tags derived from metadata:
...
Tags are useful for filtering policies in the OCI Console and for compliance automation that needs to identify OpenMCF-managed policies.
