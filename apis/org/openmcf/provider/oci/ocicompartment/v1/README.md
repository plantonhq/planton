...
- **Feed Every Downstream Resource**: The `compartmentId` output is consumed by every other OCI component in OpenMCF — OciVcn, OciSubnet, OciComputeInstance, OciAutonomousDatabase, and all others. This makes OciCompartment the root of the dependency graph for OCI deployments.
...
- **Automatic Tagging**: Standard OpenMCF freeform tags are applied to every compartment (resource kind, resource ID, organization, environment, and user-defined labels from metadata).
