...
OciFileSystem is an OpenMCF component that deploys an OCI File Storage file system together with a dedicated mount target and one or more NFS exports. It provides a single declarative manifest to create a fully functional NFS-accessible file system with per-client access control.
...
- **Foreign key references** — `compartmentId`, `subnetId`, `nsgIds`, `kmsKeyId`, and `filesystemSnapshotPolicyId` support `valueFrom` to reference other OpenMCF-managed resources.
