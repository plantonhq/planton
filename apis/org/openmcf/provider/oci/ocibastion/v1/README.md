...
OciBastion is an OpenMCF component that deploys an OCI Bastion service instance. It provides a single declarative manifest to create a managed SSH gateway with configurable client CIDR restrictions, session TTL limits, and optional DNS proxy support.
...
- **Foreign key references** — `compartmentId` and `targetSubnetId` support `valueFrom` to reference OpenMCF-managed resources.
