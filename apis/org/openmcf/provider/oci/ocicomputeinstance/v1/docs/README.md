...
**Flex shapes are the modern default.** OCI has been moving toward flex shapes as the primary offering, with fixed shapes being legacy. The OpenMCF spec reflects this by making `shapeConfig` a first-class field rather than burying it in a generic options map.
...
3. **Secondary VNICs are different.** OCI supports attaching additional VNICs to an instance, and those are independently detachable. If/when OpenMCF adds secondary VNIC support, it would be a separate resource — matching the different lifecycle.
