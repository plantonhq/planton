...
OpenMCF component for deploying an Oracle Cloud Infrastructure PostgreSQL Database System — a fully managed PostgreSQL service running on dedicated compute shapes with configurable storage durability, flexible sizing, and built-in backup policies.
...
- **Foreign key references** — reference compartments, subnets, NSGs, and Vault secrets from other OpenMCF resources via `valueFrom`
