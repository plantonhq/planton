...
OciAlarm is an OpenMCF component that deploys an OCI Monitoring Alarm. It provides a single declarative manifest to create a metric evaluation rule that triggers notifications when thresholds are breached, with support for multi-threshold overrides and configurable notification behavior.
