...
OciKmsVault is an OpenMCF component that deploys an OCI Key Management Service vault. It provides a single declarative manifest to create an HSM-backed container for encryption keys with configurable isolation levels — shared, dedicated, or external key manager.
...
- **Foreign key references** — `compartmentId` supports `valueFrom` to reference an OpenMCF-managed OciCompartment.
