...
The OCI Vault secret name is immutable after creation and must be unique within the vault. The OpenMCF metadata name is the resource identifier within the OpenMCF control plane. They may differ because the OCI name has different constraints (e.g., no prefix requirements) and the resource may be renamed in the OpenMCF catalog without recreating the OCI secret.
