...
OciVaultSecret is an OpenMCF component that deploys an OCI Vault secret. It provides a single declarative manifest to store sensitive data (credentials, certificates, API keys) in a KMS vault with encryption, lifecycle rules, and optional scheduled rotation.
