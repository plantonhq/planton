...
- **Schema drift detection is limited** — If someone modifies the table schema outside of OpenMCF (e.g., through the OCI Console), the manifest's DDL may not reflect the actual table state. Pulumi's state tracking handles the table resource itself, but DDL changes are opaque to the state model.

## Index Sub-Resource Design

### Decision

Secondary indexes are declared as a repeated `Index` message within `OciNosqlTableSpec`, not as separate top-level OpenMCF resources.
...
| Child tables (names containing `.`) | OCI NoSQL child tables inherit the parent's shard key. Managing parent-child relationships across separate OpenMCF resources requires a dependency model not yet in place. Workaround: deploy each table as a standalone resource with its own shard key. |
...
The module follows the standard OpenMCF Pulumi module pattern: `main.go` initializes locals and the OCI provider, then delegates to resource-specific functions. `nosql_table.go` creates the table, exports the table OCID, and iterates over the index list to create each secondary index with an explicit dependency on the table resource.
