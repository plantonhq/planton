...
DNS zone names are globally significant identifiers (domain names). Using `metadata.name` as the zone name keeps the OpenMCF resource identity aligned with the DNS domain. There is no separate `displayName` or `zoneName` field because the domain name is the natural identifier.
