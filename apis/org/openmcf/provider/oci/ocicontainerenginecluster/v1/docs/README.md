...
OKE's API models clusters and node pools as distinct lifecycle objects, and the OpenMCF component design follows this separation. The reasons are both technical and practical:
...
- **Defined Tags on the Cluster Itself** — The cluster resource uses OpenMCF freeform tags. OCI defined tags (namespace-scoped, schema-validated tags) require a tag namespace to be created first. Defined tag support can be added later when the tag namespace pattern is established across OCI components.
