...
Channel consumption limits prevent a single consumer channel from monopolizing queue resources in multi-tenant scenarios. While not commonly used, omitting it would prevent operators who need fine-grained throughput control from using OpenMCF for their queue deployments.
