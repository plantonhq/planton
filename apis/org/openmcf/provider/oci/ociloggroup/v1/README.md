...
OciLogGroup is an OpenMCF component that deploys an OCI Log Group with bundled logs. It provides a single declarative manifest to create the organizational container for OCI Logging service logs along with its constituent log definitions.
...
- **Foreign key references** — `compartmentId` and service log `resource` support `valueFrom` for composability with any OpenMCF component.
