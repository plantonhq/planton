...
OCI requires dynamic groups to be created in the tenancy root compartment. This is an OCI API constraint, not an OpenMCF design choice. The rationale:
...
- Teams may have existing naming conventions for dynamic groups that differ from OpenMCF's metadata naming rules.
...
- Unlike OciCompartment (whose `compartment_id` feeds every other resource), dynamic group OCIDs are rarely cross-referenced by other OpenMCF resources.

### Freeform Tags

Consistent with all OCI components in OpenMCF, dynamic groups receive freeform tags derived from metadata:
...
Tags on dynamic groups help with tenancy-wide inventory and compliance auditing — identifying which dynamic groups are managed by OpenMCF and which team owns them.
...
This loose coupling is an OCI design decision, not an OpenMCF choice. It provides flexibility (any number of policies can reference any number of dynamic groups) at the cost of no compile-time verification that the referenced group exists.
