...
OciBlockVolume is an OpenMCF component that deploys an OCI Block Volume along with an optional backup policy assignment. It provides a single declarative manifest to manage a block storage device with configurable performance tiers, autotune policies, cross-region replicas, and scheduled backups.
...
- **Foreign key references** — `compartmentId`, `kmsKeyId`, `backupPolicyId`, `xrcKmsKeyId`, and replica `xrrKmsKeyId` support `valueFrom` to reference other OpenMCF-managed resources.
