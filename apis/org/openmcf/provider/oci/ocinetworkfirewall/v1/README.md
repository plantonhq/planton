...
OciNetworkFirewall is an OpenMCF component that deploys an OCI Network Firewall with an inline firewall policy. It provides a single declarative manifest to create a network firewall appliance, its policy, and all policy sub-resources (address lists, services, service lists, URL lists, security rules).
