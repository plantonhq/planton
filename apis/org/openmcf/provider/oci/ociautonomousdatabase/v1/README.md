...
- **Foreign key references** — `compartmentId`, `subnetId`, `nsgIds`, `kmsKeyId`, `vaultId`, `secretId`, and `autonomousContainerDatabaseId` all accept `valueFrom` references to other OpenMCF-managed resources.
...
OCI offers several managed database services. Here is how Autonomous Database compares to others available in the OpenMCF catalog:
