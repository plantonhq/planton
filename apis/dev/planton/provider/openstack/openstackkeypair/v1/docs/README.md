...
The keypair name is derived from `metadata.name` rather than having a separate `spec.name` field. This simplifies the API — the Planton resource name IS the keypair name in OpenStack. This mirrors how most users think about keypairs: a named key that they reference by name when launching instances.
