...
**Provider:** aws  
**Component Path:** `apis/dev/planton/provider/aws/awscloudfront/v1/`  
**Enum Value:** 202  
**ID Prefix:** awscf
...
- Folder exists: `apis/dev/planton/provider/aws/awscloudfront/v1/`
...
```bash
cd apis/dev/planton/provider/aws/awscloudfront/v1/
go test -v
```
...
- Explains Planton's opinionated approach and rationale
...
- **Strategic Insights:** Documents why Planton's simplified 80/20 API design choices were made
...
1. **Create Pulumi Overview Document**
   ...
   - **How:** Document the module architecture (distribution.go, locals.go, outputs.go structure), explain how CloudFront-specific Pulumi resources are used, document any CloudFront-specific gotchas (us-east-1 certificate requirement), and explain the integration with Planton's stack input pattern
...
**Path to reference:** `apis/dev/planton/provider/aws/awsalb/v1/`
