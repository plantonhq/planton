...
The **AwsSnsTopic** resource provides a standardized way to provision and manage AWS SNS topics through Planton. It supports both Standard and FIFO topic types with bundled subscriptions, KMS encryption, IAM access policies, message filtering, and subscription dead letter queues.
...
When you define an AwsSnsTopic resource, Planton:
...
5. **Applies Tags**: Tags the topic with Planton metadata (organization, environment, resource kind, resource ID).
