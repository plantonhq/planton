...
1. Register the IP range with AWS via BYOIP provisioning (outside Planton scope)
...
Following the Planton bundling principle: **split resources that have independent lifecycles**. The EIP is the allocation; association is the consumer's responsibility (or a separate AwsEipAssociation component).
