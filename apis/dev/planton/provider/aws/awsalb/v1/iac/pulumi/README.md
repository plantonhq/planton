...
2. **Validate (Optional)**  
   ...
   ```bash
   planton validate --manifest awsalb.yaml
   ```
...
3. **Deploy**  
   ...
   ```bash
   # Pulumi example
   planton pulumi up --manifest awsalb.yaml --stack myorg/dev

   # Terraform example
   planton terraform apply --manifest awsalb.yaml --stack myorg/dev
   ```
...
Below are the key fields for the **AwsAlb** resource. Consult the Protobuf definitions in the Planton
