...
**Planton equivalent**: `AwsIamRole` with `trust_policy`, `managed_policy_arns`, and `inline_policies`.
...
**Planton equivalent**: `GcpServiceAccount` with `project_iam_roles` and `org_iam_roles`.
...
**Planton equivalent**: `AzureUserAssignedIdentity` with `role_assignments[].scope` and `role_assignments[].role_definition_name`.
