...
The `locals.go` file sets `DisplayName` to `spec.DisplayName` when provided, otherwise falls back to `metadata.Name`. This avoids requiring users to set both fields while still allowing explicit control when the OCI Console name should differ from the Planton resource name.
