...
Service logs can collect from many different OCI resource types: VCNs, subnets, buckets, API gateways, functions, load balancers, etc. Using a generic `StringValueOrRef` without a fixed `default_kind` allows the field to reference any Planton component via `valueFrom`, providing maximum composability across the OCI ecosystem.
