...
OCI's PostgreSQL DB System (`oci_psql_db_system`) is a standalone managed service with its own API surface, distinct from the Oracle Base Database Service (`oci_database_db_system`) and MySQL DB System (`oci_mysql_mysql_db_system`). Giving it a dedicated Planton kind allows:
