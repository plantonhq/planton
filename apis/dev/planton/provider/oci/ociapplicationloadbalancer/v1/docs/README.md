...
Oracle has deprecated `path_route_set` in favor of routing policies. The `oci_load_balancer_path_route_set` Terraform resource still exists for backward compatibility, but new deployments should use routing policies instead. The OciApplicationLoadBalancer component does not create path route sets.
...
If routing policy management becomes a common request, a future version could either bundle them or create a separate OciApplicationLoadBalancerRoutingPolicy component.
