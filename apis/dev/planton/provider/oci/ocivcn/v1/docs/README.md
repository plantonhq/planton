...
The OCI VCN component is the networking foundation for every other OCI resource in Planton. Every subnet, compute instance, database, OKE cluster, and load balancer in Oracle Cloud Infrastructure lives inside a VCN. Getting this component right — its spec shape, gateway bundling decisions, and output surface — determines the ergonomics and composability of the entire OCI provider.
...
Planton exports the OCIDs of all three defaults as stack outputs. Downstream components (OciSubnet in particular) can reference these directly or use custom replacements.
...
The practical implication: OCI's approach requires planning all CIDR blocks upfront, while AWS allows incremental expansion. Planton's `cidrBlocks` repeated field reflects OCI's native model.
...
OCI supports two tagging systems: freeform tags (key-value strings, no schema) and defined tags (schema-enforced, namespace-scoped). Planton uses freeform tags because:
...
### Tagging Includes Planton Metadata
...
- **Automatic tagging** with Planton metadata
