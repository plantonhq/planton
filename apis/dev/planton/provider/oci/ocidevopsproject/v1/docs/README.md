...
The OCI provider nests the notification topic inside a `notification_config` block with a single field (`topic_id`). Single-field wrapper blocks add YAML depth without adding meaning. Flattening to `notificationTopicId` at the spec level makes manifests more readable and matches the pattern used elsewhere in Planton for single-field provider blocks.

### Why is the project name derived from metadata.name?

OCI DevOps project names must be unique within a compartment and are immutable after creation. Using `metadata.name` as the project name keeps the naming consistent with the Planton resource identity and avoids a separate `displayName` field that could diverge from the resource name.
...
| notificationTopicId as StringValueOrRef | Composable with ONS topic resources | ONS topics are not yet an Planton component |
