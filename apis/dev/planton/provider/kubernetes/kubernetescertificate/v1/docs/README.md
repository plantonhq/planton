...
This component wraps the Certificate CR lifecycle into a single Planton resource, supporting both public TLS (via ACME ClusterIssuers) and internal PKI (via SelfSigned/CA Issuers).

## Why a Standalone Certificate Component?

Previously, Planton components that needed TLS certificates (KubernetesLocust, KubernetesArgocd, KubernetesDeployment, etc.) each created their own Certificate inline. This worked well for ingress-coupled certificates but left a gap for two important patterns:
