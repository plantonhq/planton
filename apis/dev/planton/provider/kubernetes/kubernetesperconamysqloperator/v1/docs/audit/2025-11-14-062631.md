...
**Provider:** kubernetes  
**Category:** addon  
**Component Path:** `apis/dev/planton/provider/kubernetes/kubernetesperconamysqloperator/v1/`  
**Enum Value:** 834  
**ID Prefix:** percpgop
...
1. **Missing Unit Tests (spec_test.go)**
   ...
   - **File:** `apis/dev/planton/provider/kubernetes/kubernetesperconamysqloperator/v1/spec_test.go`

2. **Terraform Module Missing Standard Files**
   - **Why it blocks:** Terraform module lacks standard structure expected by Planton CLI. Missing `locals.tf` and `outputs.tf` (inline in main.tf) deviate from conventions and make the module harder to maintain.
   - **What to do:**
...
- Folder exists at correct path: `apis/dev/planton/provider/kubernetes/kubernetesperconamysqloperator/v1/`
...
- While the inline approach works, it deviates from Planton's standard module structure
...
- Explains Planton's design philosophy and approach
...
- Clear overview from Planton perspective
...
1. **Create spec_test.go with comprehensive validation tests**
   ...
   - **Reference:** See `apis/dev/planton/provider/kubernetes/certmanager/v1/spec_test.go` for similar example
...
1. **Create Pulumi locals.go file**
   ...
   - **File:** `iac/pulumi/module/locals.go`
   - **Why:** Improves code organization and follows Planton conventions
...
3. **Separate Terraform locals and outputs**
   ...
   - **Files:** `iac/tf/locals.tf`, `iac/tf/outputs.tf`
   - **Why:** Follows Planton's standard module structure and improves maintainability
   - **How:**
