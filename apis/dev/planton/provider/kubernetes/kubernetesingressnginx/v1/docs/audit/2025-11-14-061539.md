...
- Folder exists: `apis/dev/planton/provider/kubernetes/kubernetesingressnginx/v1/`
...
```bash
cd apis/dev/planton/provider/kubernetes/kubernetesingressnginx/v1/
go test -v
```
...
**Alternative:** Run the complete flow using `@forge-planton-component` to systematically address all gaps.
...
```bash
@audit-planton-component KubernetesIngressNginx
```
