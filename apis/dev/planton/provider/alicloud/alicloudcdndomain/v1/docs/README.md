...
**AliCloudCdnDomain** Planton component (R26). The component provides a
...
- What the Planton component covers and why it makes certain trade-offs.
...
Planton modules provides compile-time type safety on argument names and
...
resources (`alicloud_cdn_domain_config`) or dedicated Planton components if
the pattern stabilizes.

### 5.2 Field Coverage

The following table maps Alibaba Cloud CDN domain fields to their Planton
coverage status:

| CDN API Field | Planton Field | Covered |
|--------------|---------------|---------|
...
These are string-valued identifiers. Planton does not validate their existence
...
This convention is consistent across all Planton Alibaba Cloud components.
...
- Validate the manifest schema: `planton validate -f manifest.yaml`.
- Preview the deployment: `planton pulumi preview` or `planton tofu plan`.
...
- `planton pulumi refresh` / `planton tofu plan` will show differences.
...
**Terraform:**
```shell
planton tofu import alicloud_cdn_domain_new.main cdn.example.com
```
