...
This document examines the full deployment landscape for SLS projects — from manual console provisioning to control-plane-based automation — and explains how Planton bundles the project-store-index triad into a single, validated API resource that eliminates the most common misconfigurations while remaining flexible enough for production use.
...
This is exactly the pattern Planton's Terraform module uses — but wrapped behind a validated API that ensures the `for_each` logic is correct and the index configuration is consistent.
...
**Planton Context**: Planton's protobuf-defined API is designed for this model. The YAML manifest is a desired-state declaration that can be applied once (CLI mode) or continuously reconciled (control-plane mode). The `AliCloudLogProject` resource is a Kubernetes-native API object, not just a CLI input format.

**Verdict**: The future of infrastructure management. Planton's API design anticipates this model even when used in CLI mode today.
...
| Planton | Yes | Yes | Yes | Proto-validated | Yes | 5 list items in YAML |

The key differentiator is the **Bundled** column. Every other method treats the project, store, and index as independent resources. Planton is the only approach that bundles them into a single validated declaration, ensuring that stores always have indexes and that the entire triad is provisioned atomically.

## The Planton Approach

### Design Philosophy: The DD07 Bundling Decision

The most important design decision for AliCloudLogProject is **DD07: composite bundling**. Instead of creating three separate Planton resources (`AliCloudLogProject`, `AliCloudLogStore`, `AliCloudLogStoreIndex`), the component bundles all three into a single resource.
...
**`projectName` vs `name`**: The spec uses `projectName` (not `name`) because the SLS API requires a globally unique project name that is distinct from the Planton `metadata.name`. The metadata name is the local resource identifier; the project name is the Alibaba Cloud resource identifier.
...
**Optional fields with proto defaults**: Every optional field uses the `(dev.planton.shared.options.default)` annotation to document its default value. The IaC modules read these defaults from the proto definition, ensuring consistency between the API contract and the implementation.

**`enableIndex` defaults to `true`**: This is an opinionated default. In raw SLS, indexes are not created automatically. Planton inverts this because the primary value of SLS is querying logs — storing unsearchable data is almost never the intent.
...
**Full-Text Index Defaults**: The Planton module creates a full-text index with:
...
**When Full-Text Isn't Enough**: Applications with deeply nested JSON logs or high-cardinality fields may benefit from field-level indexes. These are not supported by the current Planton API (see 80/20 scoping above) and should be configured directly via the SLS console or Terraform after initial provisioning.
...
**Multi-Region Strategy**: Deploy one `AliCloudLogProject` resource per region where workloads run. Use SLS's built-in cross-region replication (not managed by this Planton component) if centralized querying is needed.
...
SLS project deployment is a solved problem at every level of the tooling spectrum — from console to control plane. What Planton adds is not a new deployment mechanism but a **bundled, validated abstraction** that eliminates the most common misconfigurations:
...
The DD07 bundling decision is the architectural cornerstone: by treating project + stores + indexes as a single resource, Planton makes the common case simple (one YAML resource for a complete logging setup) while leaving the advanced case possible (custom field-level indexes and ETL can be layered on top).
