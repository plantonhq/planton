...
**Planton**: Provides a protobuf-defined API for cloud resources, supporting both Pulumi and Terraform as IaC backends. The key innovation is a **unified resource model** (apiVersion, kind, metadata, spec, status) that works the same regardless of the underlying IaC engine. A VPC defined as an Planton manifest can be deployed via Pulumi or Terraform with the same YAML.
...
Planton provides a Kubernetes-style API for deploying Alibaba Cloud VPCs that prioritizes the 80% use case while maintaining clear extensibility boundaries.
...
| `tags` | map | No | `{}` | User-defined key-value tags, merged with Planton system tags. |
...
Planton adds system tags to every resource it manages. For AliCloudVpc, the Pulumi module merges user-defined tags from `spec.tags` with system tags:
...
Planton automatically adds system tags (resource name, kind, org, env), so user tags should focus on business context.
