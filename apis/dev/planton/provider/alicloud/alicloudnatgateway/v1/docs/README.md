...
SNAT entries require the EIP's IP address string, but Planton users reference the EIP by its allocation ID. Rather than exposing two fields (`eip_id` + `eip_ip_address`), the IaC modules resolve the IP internally:
...
This is safe because Planton middleware guarantees the EIP is provisioned before the NAT Gateway component runs.
...
| Field | Reason |
| --- | --- |
| `period` | Subscription billing detail; managed outside Planton |
