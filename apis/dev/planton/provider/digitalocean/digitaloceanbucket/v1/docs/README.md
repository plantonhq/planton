...
This document examines that spectrum—from manual configuration through CLI tools to full IaC automation—and explains why Planton chose **Pulumi with the DigitalOcean provider** as the foundation for production Spaces deployments.
...
## The Planton Choice: Pulumi

Planton uses **Pulumi with the DigitalOcean provider** as the foundation for DigitalOcean Spaces deployments. Here's why:
...
Planton is an open-source project. Pulumi is open source (Apache 2.0), and while Terraform recently shifted to a Business Source License, Pulumi's commitment to open source aligns with our values. OpenTofu is also a strong open-source option, but Pulumi's programmatic approach offers unique advantages.

### 3. Multi-Language Support

Different teams prefer different languages. Pulumi supports TypeScript, Python, Go, C#, and Java—teams can use Planton's infrastructure components in their preferred language.
