...
import { InfrastructureVisual, DeliveryVisual, ReviewVisual, OwnershipVisual } from '@/components/marketing/WorkflowVisuals';
    ...
    <div className={styles.container}>
      ...
      <section className={styles.agents} aria-labelledby="agents-title">
        ...
        <figure className={styles.agentFlow}>
          <div className={styles.agentPrompt}><span aria-hidden="true">↳</span><p>“{H.agents.prompt}”</p></div>
          <ol className={styles.agentNodes}>{H.agents.nodes.map((node,i)=><li key={node}><span className={styles.mono}>0{i+1}</span><strong>{node}</strong>{i<3&&<span className={styles.agentArrow} aria-hidden="true">→</span>}</li>)}</ol>
          <figcaption className={styles.visualCaption}>{H.illustration}</figcaption>
        </figure>
        <ol className={styles.agentSteps}>{H.agents.steps.map(step=><li key={step.title}><h3>{step.title}</h3><p>{step.text}</p></li>)}</ol>
      ...
      <section className={styles.feature} aria-labelledby="infrastructure-title">
        <div className={styles.featureCopy}><p className={styles.eyebrow}>{H.infrastructure.eyebrow}</p><h2 id="infrastructure-title" className={styles.heading}>{H.infrastructure.title}</h2><p>{H.infrastructure.intro}</p>{H.infrastructure.paragraphs.map(p=><p key={p}>{p}</p>)}<p className={styles.note}>{H.infrastructure.note}</p></div>
        <InfrastructureVisual/>
      </section>
      <section className={`${styles.feature} ${styles.featureReverse}`} aria-labelledby="delivery-title">
        <DeliveryVisual/>
        <div className={styles.featureCopy}><p className={styles.eyebrow}>{H.delivery.eyebrow}</p><h2 id="delivery-title" className={styles.heading}>{H.delivery.title}</h2><p>{H.delivery.intro}</p>{H.delivery.paragraphs.map(p=><p key={p}>{p}</p>)}<p className={styles.note}>{H.delivery.note}</p></div>
      </section>
