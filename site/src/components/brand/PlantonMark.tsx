...
export interface PlantonMarkProps {
  /** Rendered height in px; width scales to the glyph's 28:32 ratio. */
  size?: number;
  className?: string;
  title?: string;
}
...
/**
 * The Planton "P" mark — the same glyph used across planton.ai. Drawn with
 * `currentColor` so it inherits text color (monochrome by default). This is the
 * canonical brand mark; do not substitute a raster.
 */
export function PlantonMark({ size = 28, className, title = "Planton" }: PlantonMarkProps) {
  const width = Math.round((size * 28) / 32);
  return (
    <svg
      width={width}
      height={size}
