...
description: "This preset creates a single-container OCI Container Instance running an HTTP service with a health check, a public IP for direct access, and an always-restart policy. It uses the CI.Standard.E4.Flex..."
type: "preset"
rank: "01"
...
- **Single VNIC, no NSG** -- Kept minimal for simplicity. Network access is controlled by the subnet's default security list. For fine-grained ingress/egress rules, add `nsgIds` referencing an `OciSecurityGroup`.
