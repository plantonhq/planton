---
title: "AzureLinuxWebApp: Research & Design Documentation"
description: "AzureLinuxWebApp: Research & Design Documentation deployment documentation"
icon: "package"
order: 100
...
This document captures the research, design rationale, and 80/20 scoping decisions behind the `AzureLinuxWebApp` OpenMCF component.
...
Private endpoints give the Web App a private IP address within the VNet, making it inaccessible from the public internet. This is modeled as a separate `AzurePrivateEndpoint` component in OpenMCF, not as a field on AzureLinuxWebApp.
...
| Field | OpenMCF Default | Azure API Default | Rationale |
|-------|----------------|-------------------|-----------|
...
Azure defaults `client_affinity_enabled` to `true` (ARR session affinity cookies). OpenMCF defaults to `false` because:
