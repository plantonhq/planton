...
- [GCP VPC](/docs/catalog/gcp/vpc) — VPC network with Private Service Access for cluster connectivity
- [GCP KMS Key](/docs/catalog/gcp/kms-key) — CMEK encryption keys for data, backups, and continuous backups
- [GCP KMS Key Ring](/docs/catalog/gcp/kms-key-ring) — Key ring containing KMS keys (must be in the same region as the cluster)
- [GCP Service Account](/docs/catalog/gcp/service-account) — Service account for application-level IAM access to AlloyDB
- [GCP Firewall Rule](/docs/catalog/gcp/firewall-rule) — Network-level access control for AlloyDB connectivity
