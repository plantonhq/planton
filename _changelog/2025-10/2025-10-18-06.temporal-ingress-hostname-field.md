...
message TemporalKubernetesSpec {
  dev.planton.shared.kubernetes.IngressSpec ingress = 6;
}
```
...
2. **Updated Field Type**: Line 42 changed from `dev.planton.shared.kubernetes.IngressSpec` to `TemporalKubernetesIngress`
...
```bash
# Preview changes
planton pulumi preview --manifest temporal.yaml
...
# Deploy
planton pulumi up --manifest temporal-test.yaml
...
# Deploy
planton pulumi up --manifest temporal-frontend-only.yaml
...
# Attempt deploy
planton pulumi up --manifest temporal-invalid.yaml
