...
[2,628 / 2,635] Running nogo on //cmd/planton:planton
...
- Calls `go_sdk.nogo(nogo = "//:planton_nogo")` to register analyzer
...
```bash
cd ~/scm/github.com/plantonhq/planton && go vet ./...
# Exit code: 0
# ✅ PASS - No errors
```
...
```bash
./bazelw build --config=bb --remote_header=x-buildbuddy-api-key=$PLANTON_BUILDBUDDY_API_KEY //:planton
```

**Output**:
```
INFO: Analyzed target //:planton (0 packages loaded, 0 targets configured).
...
```bash
~/scm/github.com/plantonhq/planton  make build
...
```
[2,628 / 2,635] Running nogo on //cmd/planton:planton; 1s darwin-sandbox
INFO: 524 total actions
```

**Subsequent Builds** (warm cache):
```
[2,628 / 2,635] Running nogo on //cmd/planton:planton; 0s remote-cache
...
**3. MODULE.bazel**:
- Added `go_sdk.nogo(nogo = "//:planton_nogo")` call
...
bazel build --define=nogo=off //:planton

# Enable only critical analyzers
bazel build --define=nogo=critical //:planton
```
