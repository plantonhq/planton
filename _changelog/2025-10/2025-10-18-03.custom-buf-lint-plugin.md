...
3. Check if field has `(dev.planton.shared.options.default)` extension
...
```
cloud/planton/apis/connect/awscredential/v1/spec.proto:68:3:Field "region" has a default value but is not marked as optional. Scalar fields with (dev.planton.shared.options.default) must use the 'optional' keyword to enable proper field presence detection.

Fix: optional string region = 4 [(dev.planton.shared.options.default) = "..."]; (buf-plugin-planton)
```
...
- Matches pattern used in planton monorepo
...
message AwsProviderConfig {
  ...
  string region = 4 [(dev.planton.shared.options.default) = "us-west-2"];
}
```
...
message AwsProviderConfig {
  ...
  optional string region = 4 [(dev.planton.shared.options.default) = "us-west-2"];
}
```
...
message ExampleSpec {
  ...
  string namespace = 1 [(dev.planton.shared.options.default) = "default"];
  ...
  bool enabled = 3 [(dev.planton.shared.options.default) = "true"];
}
```
...
```protobuf
string region = 7 [(dev.planton.shared.options.recommended_default) = "us-west-2"];
// OK: recommended_default doesn't require optional (different extension)
```

✅ **Optional fields with defaults** (correct usage):
```protobuf
optional string version = 8 [(dev.planton.shared.options.default) = "v1.0.0"];
optional int32 timeout = 9 [(dev.planton.shared.options.default) = "30"];
optional bool debug = 10 [(dev.planton.shared.options.default) = "false"];
...
- Local `planton` module - For accessing custom options extension
...
import (
	...
	"github.com/plantonhq/planton/buf/lint/planton/rules"
)
...
- Simplified dependency management
- Consistent with planton monorepo structure

### Dependency Management

**Plugin go.mod**:
```go
module github.com/plantonhq/planton/buf/lint/planton
...
**Before**:
```protobuf
string username = 9 [(dev.planton.shared.options.default) = "master"];
```

**After**:
```protobuf
optional string username = 9 [(dev.planton.shared.options.default) = "master"];
```
...
grep -r "(dev.planton.shared.options.default)" apis/ | wc -l
...
```bash
buf registry plugin publish buf.build/planton/buf-plugin-planton
```
...
```protobuf
string field = 1 [(dev.planton.shared.options.default) = "value"];
int32 port = 2 [(dev.planton.shared.options.default) = "443"];
bool enabled = 3 [(dev.planton.shared.options.default) = "false"];
```

✅ **DO** (Lint will pass):
```protobuf
optional string field = 1 [(dev.planton.shared.options.default) = "value"];
optional int32 port = 2 [(dev.planton.shared.options.default) = "443"];
optional bool enabled = 3 [(dev.planton.shared.options.default) = "false"];
```
...
Fix: optional string field = 1 [(dev.planton.shared.options.default) = "value"];
```
...
- Matches pattern from planton monorepo
