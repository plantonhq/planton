...
When running Pulumi commands through `planton`, users were seeing plain streaming output instead of Pulumi's beautiful interactive tree view that shows resources in a hierarchical structure:

**What Users Were Getting** (Plain Streaming Output):
```
Previewing update (planton/kubernetes.PostgresKubernetes.test-swarup)

@ Previewing update.....
    pulumi:pulumi:Stack planton-kubernetes.PostgresKubernetes.test-swarup  Compiling the program ...
@ Previewing update....
    pulumi:pulumi:Stack planton-kubernetes.PostgresKubernetes.test-swarup  Finished compiling
 +  pulumi:pulumi:Stack planton-kubernetes.PostgresKubernetes.test-swarup create Finished compiling
@ Previewing update....
...
- Frustration when comparing planton output to native Pulumi experience
...
**Command**:
```bash
planton pulumi preview --manifest postgres.yaml --module-dir ${MODULE}
```
...
**Command**:
```bash
planton pulumi preview --manifest postgres.yaml --module-dir ${MODULE} --diff
```
...
**Command**:
```bash
planton pulumi up --manifest postgres.yaml --module-dir ${MODULE} --yes
```
...
**File**: `cmd/planton/root/pulumi.go`
...
**Files**:
- `cmd/planton/root/pulumi/preview.go`
- `cmd/planton/root/pulumi/update.go`
- `cmd/planton/root/pulumi/destroy.go`
- `cmd/planton/root/pulumi/refresh.go`
...
**Before**: planton forced non-interactive mode, diverging from standard Pulumi UX

**After**: planton respects Pulumi's output modes, providing the same experience as native Pulumi CLI
...
```
User runs planton pulumi command
    ↓
Is --yes flag present?
    ├─ Yes → Add --yes flag
...
- Interactive tree view rendering happens in Pulumi CLI (no overhead for planton)
...
planton pulumi preview --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE}
```
...
```bash
planton pulumi preview --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE} --diff
```
...
```bash
planton pulumi up --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE} --yes
```
...
**Result**: ✅ Binary rebuilt and installed to `~/bin/planton`
...
2. **`cmd/planton/root/pulumi.go`**: Registered `--diff` flag (1 line)
...
4. **`cmd/planton/root/pulumi/preview.go`**: Read and pass `--diff` flag (2 lines added)
5. **`cmd/planton/root/pulumi/update.go`**: Read and pass `--diff` flag (2 lines added)
6. **`cmd/planton/root/pulumi/destroy.go`**: Read and pass `--diff` flag (2 lines added)
7. **`cmd/planton/root/pulumi/refresh.go`**: Read and pass `--diff` flag (2 lines added)
...
planton pulumi preview --manifest file.yaml

# To see what changed: Add --diff
planton pulumi preview --manifest file.yaml --diff

# For rapid iteration: Auto-approve with visibility
planton pulumi up --manifest file.yaml --yes

# For maximum visibility: Combine flags
planton pulumi up --manifest file.yaml --yes --diff
```
...
which planton
# Should be: ~/bin/planton

# 4. Don't redirect output
planton pulumi preview ... > output.txt  # ❌ Breaks TTY
planton pulumi preview ...                # ✅ Works
```
...
```bash
planton pulumi preview --output-format tree    # Interactive tree (default)
planton pulumi preview --output-format stream  # Streaming (force)
planton pulumi preview --output-format json    # JSON output
```
...
metadata:
  labels:
    pulumi.planton.dev/output-format: tree
    pulumi.planton.dev/show-diff: "true"
```
...
```bash
planton pulumi preview --manifest file.yaml --save-output preview.log
