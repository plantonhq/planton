...
message ExternalDnsKubernetesSpec {
  string namespace = 2 [(dev.planton.shared.options.default) = "external-dns"];
  string external_dns_version = 3 [(dev.planton.shared.options.default) = "v0.19.0"];
  string helm_chart_version = 4 [(dev.planton.shared.options.default) = "1.19.0"];
}
```
...
message TestCloudResourceOneSpec {
  // String fields
  string string_field = 1 [(dev.planton.shared.options.default) = "default-string"];
  string string_no_default = 2;
  
  // Numeric fields with defaults
  int32 int32_field = 3 [(dev.planton.shared.options.default) = "42"];
  int64 int64_field = 4 [(dev.planton.shared.options.default) = "9999"];
  uint32 uint32_field = 5 [(dev.planton.shared.options.default) = "100"];
  uint64 uint64_field = 6 [(dev.planton.shared.options.default) = "50000"];
  
  // Float fields with defaults
  float float_field = 7 [(dev.planton.shared.options.default) = "3.14"];
  double double_field = 8 [(dev.planton.shared.options.default) = "2.718"];
  
  // Bool field with default
  bool bool_field = 9 [(dev.planton.shared.options.default) = "true"];
...
message TestNestedMessage {
  string nested_string = 1 [(dev.planton.shared.options.default) = "nested-default"];
  int32 nested_int = 2 [(dev.planton.shared.options.default) = "99"];
}
```
...
planton load-manifest external-dns.yaml \
...
6. **Extract default option**: Get `dev.planton.shared.options.default` extension
...
bazel run //:planton -- load-manifest \
  ~/scm/github.com/plantonhq/planton/ops/organizations/planton/infra-hub/cloud-resources/app-prod/kubernetes/addon/external-dns/external-dns.planton.ai.yaml
```
...
metadata:
  ...
  env: app-prod
  labels:
    pulumi.planton.dev/stack.name: app-prod.ExternalDnsKubernetes.external-dns-planton
spec:
  targetCluster:
...
string my_field = 1 [(dev.planton.shared.options.default) = "my-default"];
```
...
```protobuf
string my_field = 1 [(dev.planton.shared.options.default) = ""];
```
...
```protobuf
int32 my_field = 1 [(dev.planton.shared.options.default) = "not-a-number"];
```
...
```protobuf
MyEnum my_field = 1 [(dev.planton.shared.options.default) = "INVALID_VALUE"];
```
...
message NestedMessage {
  string field = 1 [(dev.planton.shared.options.default) = "default"];
}
```
...
```protobuf
optional bool my_field = 1 [(dev.planton.shared.options.default) = "true"];
```
...
Support `dev.planton.shared.options.recommended_default` for suggestions:

```protobuf
string region = 1 [
  (dev.planton.shared.options.default) = "us-east-1",
  (dev.planton.shared.options.recommended_default) = "us-west-2"
];
```
...
```protobuf
string version = 1 [
  (dev.planton.shared.options.default_if) = {
    field: "environment"
    value: "production"
...
```protobuf
message Parent {
  string version = 1 [(dev.planton.shared.options.default) = "v1.0.0"];
}

message Child {
  string version = 1 [(dev.planton.shared.options.inherit_default_from) = "Parent.version"];
}
```
...
```bash
planton load-manifest manifest.yaml --no-defaults
```
...
```bash
planton docs generate-defaults ExternalDnsKubernetes
...
**Planton Approach**: Uses proto field options (closest to the API definition, type-safe, language-agnostic).
...
The `dev.planton.shared.options.default` extension accepts only string values:
...
5. Contact Planton support for assistance
