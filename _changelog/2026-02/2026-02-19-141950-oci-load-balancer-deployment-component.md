...
A single `OciLoadBalancer` component that bundles the full load balancer stack into one deployment unit, following the same bundling philosophy as `OciVcn` (which bundles gateways).
...
```
apis/org/openmcf/provider/oci/ociloadbalancer/v1/
...
Kind registered as `OciLoadBalancer = 3320` in `cloud_resource_kind.proto`.
